...
void SimuladorRed::tick(Grafo& g, float dt) {
...
std::vector<std::pair<int, std::string>> SimuladorRed::tablaRuteo(const Grafo& g, int nodo_id) {
...
void SimuladorRed::enviarFlujo(int origen, int destino, float mbps,
...
float SimuladorRed::usoArista(int origen, int destino) const {
...
float SimuladorRed::perdidaEnArista(int origen, int destino) const {
    auto key = std::make_pair(origen, destino);
    if (!estado.aristas.count(key)) return 0.0f;
    return estado.aristas.at(key).packet_loss;
}

float SimuladorRed::tiempoReal() {
    static auto t0 = std::chrono::steady_clock::now();
    return std::chrono::duration<float>(std::chrono::steady_clock::now() - t0).count();
}

void SimuladorRed::notificar(const std::string& msg, uint32_t color, float duracion) {
    estado.notificaciones.push_back({tiempoReal(), duracion, msg, color});
}

void SimuladorRed::simularFalloNodo(int nodo_id, Grafo& g) {
...
void SimuladorRed::simularFalloArista(int origen, int destino, Grafo& g) {
    auto key = std::make_pair(origen, destino);
...
void SimuladorRed::restaurarNodo(int nodo_id, Grafo& g) {
    if (!estado.nodos.count(nodo_id)) return;
...
void SimuladorRed::restaurarArista(int origen, int destino, Grafo& g) {
    auto key = std::make_pair(origen, destino);
...
void SimuladorRed::enviarFlujoPreset(Grafo& g, int preset) {
...
void SimuladorRed::enviarTraceroute(Grafo& g, int origen, int destino) {
...
void SimuladorRed::simularTormenta(Grafo& g, float porcentaje, float duracion) {
...
std::vector<int> SimuladorRed::obtenerRuta(const Grafo& g, int origen, int destino) {
...
void SimuladorRed::colectarEstadisticas(Grafo& g) {
    float tp_total = 0.0f;
...
void SimuladorRed::actualizarNodos(Grafo& g, float dt) {
    for (const auto& n : g.nodos) {
...
void SimuladorRed::actualizarAristas(Grafo& g, float dt) {
    (void)g;
    for (auto& [key, ea] : estado.aristas) {
        if (!ea.activa) continue;
...
void SimuladorRed::procesarColas(Grafo& g, float dt) {
    timer_paquetes += dt;
...
void SimuladorRed::generarEventos(Grafo& g, float dt) {
